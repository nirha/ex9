...
class Board:
    ...
    def __init__(self):
        ...
        l, c = 7, 7
        self.__board = [["_" for x in range(l)] for y in range(c)]
    ...
    def cell_list(self):
        ...
        for row in range(0, len(self.__board)):
            for col in range(0, len(self.__board)):
    ...
    def is_cor_in_bound(self, cor):
        ...
        if cor[0] < 0 or cor[1] < 0 or \
                cor[0] > len(self.__board) - 1:
            return False

        if cor[0] != self.target_location()[0] and \
                cor[1] > len(self.__board[0]) - 1:
            return False

        # it could be in the target location:
        if cor[0] == self.target_location()[0] and \
                cor[1] > len(self.__board[self.target_location()[0]]) - 1:
            return False
        return True
    ...
    def target_location(self):
        ...
        return int((len(self.__board) - 1) / 2), len(self.__board[0])
    ...
    def add_car(self, car):
        ...
        # implement your code and erase the "pass"
        if car.get_name() in self.__car_names:
        ...
        self.__car_names.append(car.get_name())
        for point in car.car_coordinates():
            # updating the board
            self.__board[point[0]][point[1]] = car.get_name()
        return True
    ...
    def remove_car(self, car):
        ...
        for point in car.car_coordinates():
            # updating the board
            self.__board[point[0]][point[1]] = "_"
        for name in self.__car_names:
            if name == car.get_name():
                self.__car_names.remove(name)
